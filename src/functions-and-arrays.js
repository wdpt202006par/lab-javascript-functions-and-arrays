...
function uniquifyArray (someArrayOfUniqWords){
  if(someArrayOfUniqWords.length === 0) {
    return null;
  }
  var newUniqArray = [];
  for(let i = 0; i < someArrayOfUniqWords.length; i++){
   if(someArrayOfUniqWords.indexOf(someArrayOfUniqWords[i]) === i ){
     newUniqArray.push(someArrayOfUniqWords[i]);
   }
   return newUniqArray;
  }
 }
...
function doesWordExist (someArr2Search){
  if(someArr2Search.length === 0){
    return null;
  }
  for(let i = 0; i < someArr2Search.length; i++){
     var question = prompt("Rechercher si le mot suivant existe :");
     if(question == someArr2Search[i]){
       console.log ("true");
       break;
     } else {
       console.log ("false");
...
function howManyTimes(array2Count, word2search){
  if(array2Count.length === 0){
    return null;
  }
  var numberOfTimes = 0;
  for (let i = 0; i < array2Count.length; i++) {
    if(word2search === array2Count[i]) {
      numberOfTimes += 1;
    }    
  }
